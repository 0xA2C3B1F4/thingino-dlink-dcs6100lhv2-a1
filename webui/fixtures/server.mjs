...
async function serveStatic(response, pathname) {
  ...
  try {
    ...
    const type = { ".html": "text/html; charset=utf-8", ".js": "text/javascript; charset=utf-8", ".css": "text/css; charset=utf-8", ".webmanifest": "application/manifest+json" }[extname(path)] || "application/octet-stream";
...
const server = createServer(async (request, response) => {
  ...
  try {
    ...
    if (url.pathname === "/api/v1/auth/session") {
      ...
      return json(response, 200, { authenticated, username: authenticated ? "root" : null, is_default_password: authenticated, client_ip: request.socket.remoteAddress || "127.0.0.1", control_api: { name: "Thingino Control", version: 1 } });
    }
    if (url.pathname === "/api/v1/auth/login" && request.method === "POST") {
      ...
      return json(response, 200, { success: true, is_default_password: true }, { "Set-Cookie": `thingino_session=${token}; Path=/; Max-Age=86400; HttpOnly; SameSite=Strict` });
    }
