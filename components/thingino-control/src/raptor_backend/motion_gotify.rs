...
const MESSAGE: &[u8] = b"{\"title\":\"Motion\",\"message\":\"Motion detected.\"}";
...
pub(super) trait Transport: Send + Sync {
    ...
    fn post(&self, endpoint: &str, token: &str, body: &[u8]) -> Result<u16, ()>;
}
...
struct CurlGotifyTransport {
    curl: super::motion_webhook::CurlTransport,
}

impl CurlGotifyTransport {
    fn load() -> Self {
        Self {
...
impl Transport for CurlGotifyTransport {
    fn available(&self) -> bool {
    ...
    fn post(&self, endpoint: &str, token: &str, body: &[u8]) -> Result<u16, ()> {
        ...
        let authentication = (!token.is_empty()).then(|| authentication_header(token));
        let mut headers = vec!["Content-Type: application/json"];
        if let Some(authentication) = authentication.as_deref() {
            headers.push(authentication);
        }
        let (status, response) = self
            .curl
            .post_with_headers_and_response(endpoint, body, &headers)?;
        if (200..300).contains(&status) && !gotify_acknowledged(&response) {
            return Err(());
        }
        Ok(status)
    }
...
fn authentication_header(token: &str) -> String {
    format!("X-Gotify-Key: {token}")
}

fn gotify_acknowledged(body: &[u8]) -> bool {
...
impl Service {
    pub(super) fn new() -> Self {
        Self::with_transport(Arc::new(CurlGotifyTransport::load()))
    }
    ...
    pub(super) fn start(&self) -> Result<(), BackendError> {
        ...
        *worker = Some(
            thread::Builder::new()
                .name("control-motion-gotify".to_owned())
    ...
    pub(super) fn apply(&self, config: Config) -> Result<(), BackendError> {
        if config.enabled
            && (!self.transport.available()
                ...
                || (!config.token.is_empty() && !config.endpoint.starts_with("https://")))
        {
    ...
    fn public_config(&self, saved: &Config) -> Result<BackendResponse, BackendError> {
        ...
        Ok(BackendResponse::json(
            format!(
                "{{\"saved_enabled\":{},\"enabled\":{},\"endpoint\":null,\"endpoint_set\":{},\"live_endpoint_set\":{},\"token\":null,\"token_set\":{},\"live_token_set\":{},\"matches_saved\":{},\"transport_available\":{}}}\n",
                saved.enabled,
                live.enabled,
                !saved.endpoint.is_empty(),
                !live.endpoint.is_empty(),
...
fn worker_loop(
    ...
    while !shutdown.load(Ordering::Acquire) {
        ...
        match transport.post(&config.endpoint, &config.token, MESSAGE) {
...
impl RaptorBackend {
    pub(super) fn motion_gotify_request(
        &self,
        method: &str,
        target: &str,
        body: &[u8],
        ...
        if !matches!(
            target,
            "/api/v1/config/motion-gotify" | "/api/v1/runtime/motion-gotify"
        ) {
            return None;
        }
        Some((|| match target {
            "/api/v1/config/motion-gotify" if method == "GET" && body.is_empty() => {
                self.ensure_exclusive_owner()?;
                let saved = Config::from_value(&self.host.motion_gotify_config()?);
                self.motion_gotify.public_config(&saved)
            }
            "/api/v1/config/motion-gotify" if method == "POST" && !body.is_empty() => {
                self.ensure_exclusive_owner()?;
                let _mutation = self.lock_mutation()?;
                self.host.motion_gotify_config_request("POST", body)?;
                let config = Config::from_value(&self.host.motion_gotify_config()?);
                let enabled = config.enabled;
                self.motion_gotify.apply(config).map_err(|_| BackendError::PartialApply(
                    "gotify settings were saved but could not be applied. Reload the saved and live status before retrying.",
                ))?;
                if enabled && !self.motion_gotify.runtime.running.load(Ordering::Acquire) {
                    return Err(BackendError::PartialApply(
                        "gotify settings were saved but its delivery worker is not running. Reload the saved and live status before retrying.",
                    ));
                }
                Ok(BackendResponse::json(
            ...
            "/api/v1/runtime/motion-gotify" if method == "GET" && body.is_empty() => {
                self.ensure_exclusive_owner()?;
                Ok(self.motion_gotify.runtime())
            }
            _ => Err(BackendError::Protocol),
        })())
    }

    pub(super) fn start_motion_gotify(&self) {
        let config = self
            .host
            .motion_gotify_config()
            .map(|value| Config::from_value(&value))
            .unwrap_or_default();
        if self.motion_gotify.apply(config).is_err() {
            let _ = self.motion_gotify.apply(Config::default());
        }
        let _ = self.motion_gotify.start();
    }
}
...
mod tests {
    ...
    fn config_body(
        ...
        let root = task_temp("motion-gotify-config");
        let config = root.join("thingino.json");
        fs::write(
            &config,
            format!(
                "{{\"motion_gotify\":{{\"enabled\":true,\"endpoint\":\"{saved_endpoint}\",\"{}\":\"{saved_token}\"}}}}",
                "token"
            ),
        )
        .unwrap();
        ...
        fixture.motion_gotify = Arc::new(Service::with_transport(Arc::new(AvailableTransport)));
        fixture
            .motion_gotify
            .apply(Config {
                enabled: true,
                endpoint: live_endpoint.to_owned(),
                token: live_token.to_owned(),
            })
            .unwrap();
        let response = fixture
            .motion_gotify_request("GET", "/api/v1/config/motion-gotify", b"")
            .unwrap()
            .unwrap();
    ...
    #[test]
    fn curl_posts_readable_json_to_loopback_without_authorization() {
        let transport = CurlGotifyTransport::load();
        ...
        assert!(request.ends_with("{\"title\":\"Motion\",\"message\":\"Motion detected.\"}"));
        assert_eq!(
            authentication_header("__SET_LOCALLY__"),
            "X-Gotify-Key: __SET_LOCALLY__"
        );
        assert!(gotify_acknowledged(br#"{"id":7}"#));
        assert!(!gotify_acknowledged(br#"{"id":0}"#));
        assert!(!gotify_acknowledged(br#"{"message":"accepted"}"#));
        assert_eq!(
            transport.post("http://127.0.0.1/message", "secret", MESSAGE),
            Err(())
        );
    }
    ...
    fn disabling_cancels_queued_events_before_another_request() {
        ...
        service
            .apply(Config {
                enabled: true,
                endpoint: "https://gotify.example.test/message".to_owned(),
    ...
    fn endpoint_change_discards_an_event_queued_for_the_old_generation() {
        ...
        service
            .apply(Config {
                enabled: true,
                endpoint: "https://old.example.test/message".to_owned(),
                token: "old-secret".to_owned(),
            })
            .unwrap();
        service.start().unwrap();
        assert_eq!(
        ...
        service
            .apply(Config {
                enabled: true,
                endpoint: "https://new.example.test/message".to_owned(),
                token: "new-secret".to_owned(),
            })
            .unwrap();
        let (lock, wake) = &*state;
        *lock.lock().unwrap() = true;
