...
impl PrudyntBackend {
    pub(in crate::camera) fn access_config(&self) -> Result<BackendResponse, BackendError> {
        let onvif = json::parse(&read_bounded(&self.paths.onvif_config, FILE_LIMIT)?)
            .map_err(|_| BackendError::Protocol)?;
        let prudynt = json::parse(&read_bounded(&self.paths.prudynt_config, FILE_LIMIT)?)
            .map_err(|_| BackendError::Protocol)?;
        let password_set = onvif
            .get_path("server.password")
            .and_then(Value::as_str)
            .or_else(|| prudynt.get_path("rtsp.password").and_then(Value::as_str))
        ...
        json_response(object([
            (
                ...
                onvif
                    ...
                    .and_then(Value::as_str)
                    .or_else(|| prudynt.get_path("rtsp.username").and_then(Value::as_str))
    ...
    pub(in crate::camera) fn update_access_config(
        ...
        let onvif_original = read_bounded(&self.paths.onvif_config, FILE_LIMIT)?;
        let mut prudynt = json::parse(&prudynt_original).map_err(|_| BackendError::Protocol)?;
        let mut onvif = json::parse(&onvif_original).map_err(|_| BackendError::Protocol)?;
        ...
        for (field, path, update) in [
            ...
            if let Some(endpoint) = fields.get(field).and_then(Value::as_str) {
                ...
                update_url_endpoint(&mut onvif, profile, endpoint)?;
            }
        }
