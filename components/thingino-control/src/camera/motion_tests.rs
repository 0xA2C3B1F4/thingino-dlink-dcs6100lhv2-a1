use super::super::motion_datagram::{
    MAX_INGRESS_BYTES, MotionObservation, ObservationState, PROTOCOL_VERSION, number_u64,
    parse_observation,
};
use super::super::motion_events::{MotionEvent, MotionEventDisposition};
use super::super::motion_queue::QueuePushResult;
use super::super::motion_state::{MotionConfig, MotionTransition};
use super::*;
...
#[test]
fn concurrent_start_and_shutdown_refuse_transition_until_both_threads_are_joined() {
    let (root, service) = lifecycle_service("join-transition");
    let (release_receiver, wait_receiver) = mpsc::channel();
    let (release_worker, wait_worker) = mpsc::channel();
    {
        let mut lifecycle = service.lock_lifecycle();
        lifecycle.state = MotionLifecycleState::Running;
        lifecycle.receiver = Some(thread::spawn(move || {
            wait_receiver.recv_timeout(Duration::from_secs(2)).unwrap();
        }));
        lifecycle.worker = Some(thread::spawn(move || {
            wait_worker.recv_timeout(Duration::from_secs(2)).unwrap();
        }));
    }
    ...
    // Acquiring this mutex while join is blocked also checks the lock boundary.
    let lifecycle = service.lock_lifecycle();
    assert!(lifecycle.receiver.is_none());
    assert!(lifecycle.worker.is_none());
    drop(lifecycle);
    for error in [
    ...
    assert_eq!(lifecycle_state(&service), MotionLifecycleState::Stopping);
    assert!(!shutdown.is_finished());
...
#[test]
fn manifest_recovery_rejects_invalid_paths_without_replacing_pending_clips() {
    for (field, replacement) in [
        ("id", Value::String("motion-../outside".to_owned())),
        (
            "partial_path",
            Value::String("/outside/.motion-1.partial".to_owned()),
        ),
        (
            "final_path",
            Value::String("/outside/motion-1.mp4".to_owned()),
        ),
        ("channel", number(2)),
        ("extra", Value::Bool(true)),
    ] {
        let (root, service) = lifecycle_service("manifest-rejection");
        let partial = root.join(".motion-1.partial");
        fs::write(&partial, b"partial-clip").unwrap();
        let mut pending = VecDeque::from([PendingClip {
            id: "motion-1".to_owned(),
            partial_path: partial.clone(),
            final_path: root.join("motion-1.mp4"),
            channel: 0,
            created_unix_ms: 1,
            deadline_unix_ms: u64::MAX,
        }]);
        service.persist_pending_clips(&pending).unwrap();
        let mut document =
            json::parse(&fs::read(&service.paths.motion_clip_manifest).unwrap()).unwrap();
        ...
        invalid.set_path(field, replacement).unwrap();
        entries.push(invalid);
        document.set_path("clips", Value::Array(entries)).unwrap();
        fs::write(&service.paths.motion_clip_manifest, document.to_json()).unwrap();
        pending[0].id = "motion-existing".to_owned();
        *service.pending_clips.lock().unwrap() = pending;

        service.recover_pending_clips_from(&root);

        let pending = service.pending_clips.lock().unwrap();
        assert_eq!(pending.len(), 1, "invalid {field}");
        assert_eq!(pending[0].id, "motion-existing");
        assert_eq!(pending[0].deadline_unix_ms, u64::MAX);
        assert_eq!(service.snapshot().clips_recovered, 0);
        assert_eq!(service.snapshot().clip_manifest_errors, 1);
        assert_eq!(fs::read(&partial).unwrap(), b"partial-clip");
        assert!(!root.join("motion-1.mp4").exists());
        drop(pending);
        fs::remove_dir_all(root).unwrap();
    }
}
...
#[test]
fn active_clip_waits_for_recorder_before_activation_and_manifest_removal() {
    let (root, service) = lifecycle_service("active-clip");
    let partial = root.join(".motion-1.partial");
    let final_path = root.join("motion-1.mp4");
    fs::write(&partial, b"active-clip").unwrap();
    fs::create_dir_all(service.paths.recorder_ch0_active.parent().unwrap()).unwrap();
    fs::write(
        &service.paths.recorder_ch0_active,
        format!("path={}\nduration=10\n", partial.display()),
    )
    .unwrap();
    service
    ...
    assert_eq!(service.pending_clips.lock().unwrap().len(), 1);
    assert_eq!(service.snapshot().clips_ready, 0);
    assert_eq!(service.snapshot().sink_queued, 0);
    assert_eq!(
        fs::metadata(&service.paths.motion_clip_manifest)
            .unwrap()
            .permissions()
            .mode()
            & 0o777,
    ...
    fs::remove_file(&service.paths.recorder_ch0_active).unwrap();
    service.poll_media_lifecycle();
    assert!(!partial.exists());
    assert_eq!(fs::read(final_path).unwrap(), b"active-clip");
    assert!(service.pending_clips.lock().unwrap().is_empty());
    assert!(!service.paths.motion_clip_manifest.exists());
    assert_eq!(service.snapshot().clips_ready, 1);
    assert_eq!(service.snapshot().sink_queued, 1);
    fs::remove_dir_all(root).unwrap();
}
