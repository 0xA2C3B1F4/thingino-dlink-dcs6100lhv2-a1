...
#[derive(Debug)]
pub(crate) struct MotionStateMachine {
    config: MotionConfig,
    active: bool,
    monitoring: bool,
...
impl MotionStateMachine {
    pub(crate) fn new(config: MotionConfig, recovered_active: bool) -> Self {
        Self {
            config,
    ...
    pub(crate) fn apply(&mut self, observation: MotionObservation) -> Option<MotionTransition> {
    ...
    fn apply_clear(&mut self, observation: MotionObservation) -> Option<MotionTransition> {
        ...
        if self.active && self.last_qualifying_motion_ms.is_none() {
            // Control may have restarted while Raptor and an active event kept
