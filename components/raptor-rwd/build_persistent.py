...
PRUDYNT_GUARDED_START = PRUDYNT_START[:-1] + b" || return 1\n"
PRUDYNT_GUARDED_RING_START = PRUDYNT_RING_START[:-1] + b" || return 1\n"
RESET_CONFLICTS = (
...
def patch_prudynt_init(raw: bytes) -> bytes:
    starts = (PRUDYNT_START, PRUDYNT_GUARDED_START)
    lines = [line.lstrip(b"\t") for line in raw.splitlines(keepends=True)]
    if sum(lines.count(start) for start in starts) != 1 or b"PRUDYNT_RAPTOR_RING=" in raw:
        raise PersistentCandidateError("Prudynt init does not match the proven baseline")
    start = next(start for start in starts if start in lines)
    return raw.replace(start, b"PRUDYNT_RAPTOR_RING=1 " + start)
...
def _pack(
    *,
    ...
    if len(raw) > partition_limit:
        raise PersistentCandidateError(
            f"WebRTC rootfs size {len(raw)} exceeds fixed system limit {partition_limit}"
        )
...
def _remove_unused_route_library(root: Path) -> None:
    """Drop only the unreferenced libnl routing component from a split image.

    Wi-Fi uses libnl core/genl, not route. Check both ELF dependency names
    and literal dlopen/script references before removing this optional file.
    """
    library = root / "usr/lib/libnl-route-3.so.200.26.0"
    aliases = [library.parent / name for name in (
        "libnl-route-3.so", "libnl-route-3.so.200",
    )]
    candidates = {library, *aliases}
    if not any(path.exists() or path.is_symlink() for path in candidates):
        return
    if library.is_symlink() or not library.is_file():
        raise PersistentCandidateError("optional route library changed type")
    for path in aliases:
        if not path.is_symlink() or os.readlink(path) != library.name:
            raise PersistentCandidateError("optional route library alias changed")
    for path in root.rglob("*"):
        if path in candidates or path.is_symlink() or not path.is_file():
            continue
        if b"libnl-route-3.so" in path.read_bytes():
            raise PersistentCandidateError("optional route library has a runtime reference")
    for path in aliases:
        path.unlink()
    library.unlink()
...
def _validate_universal_tree(root: Path, *, require_rwd: bool = True) -> None:
    ...
    for relative in (
        ...
        path = root / relative
        # The accepted RTSP/MJPEG base has no RWD service yet. Only the
        # pre-overlay audit permits its absence; the completed image must
        # contain the disabled service and still pass the full audit.
        if (
            relative == "etc/init.d/S96rwd" and not require_rwd
...
def build_persistent_root(
    *,
    ...
    try:
        ...
        if artifact_scope == "model-universal":
            _validate_universal_tree(root, require_rwd=False)
        else:
            validate_final_root(
                sanitized_raw,
