...
def elf32_relocatable(*, machine: int = 8, flags: int = 0x50001001) -> bytes:
    raw = bytearray(52 + 40)
    identity = b"\x7fELF\x01\x01\x01" + b"\0" * 9
    struct.pack_into(
        "<16sHHIIIIIHHHHHH",
...
def make_bundle(root: Path) -> tuple[Path, Path, dict[str, bytes]]:
    ...
    for entry in catalog["files"]:
        if entry["format"] == "elf-shared":
            raw = elf32_shared(
                soname=entry["elf"]["soname"], needed=entry["elf"]["needed"]
            )
        elif entry["format"] == "elf-relocatable":
            raw = elf32_relocatable()
        else:
            raw = b"opaque-camera-media-input"
        entry["size"] = len(raw)
...
class VendorBundleTests(unittest.TestCase):
    def test_exact_camera_local_closure_is_accepted(self) -> None:
        with tempfile.TemporaryDirectory() as directory_name:
            ...
            self.assertIsNotNone(bundle.artifacts[0].elf)
    ...
    def test_extractor_requires_exact_read_only_mtd3_mount(self) -> None:
        with tempfile.TemporaryDirectory() as directory_name:
            ...
            for entry in catalog["files"]:
                path = stock / entry["source_path"]
                path.parent.mkdir(parents=True, exist_ok=True)
                path.write_bytes(raws[entry["name"]])
