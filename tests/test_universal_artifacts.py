...
class UniversalArtifactTests(unittest.TestCase):
    ...
    def test_universal_staging_reactivates_verified_passive_install_set(self) -> None:
        with tempfile.TemporaryDirectory() as directory_name:
            root = Path(directory_name)
            authorization = SimpleNamespace(
                raw_manifest=b"authorization",
            ...
            (root / "STAGE1.PKG").write_bytes(validated.bootstrap)
            (root / "THINGINO2.BIN").write_bytes(validated.stage2.raw)
            preflight = SimpleNamespace(
                physical_device="/dev/test-media",
                mount_root=root.resolve(),
            )

            def reactivate(**_values: object) -> dict[str, str]:
                self.assertEqual(
                    (root / PROVISIONING_CARD_NAME).read_bytes(),
                    validated.provisioning_data,
                )
                return {"bootstrap": "reactivated"}
            ...
            with (
                ...
                mock.patch("installer.universal_install.media._sync_directory"),
                mock.patch(
                    "installer.universal_install.media.activate_staged_install_set",
                    side_effect=reactivate,
                ) as activate,
                mock.patch(
                    "installer.universal_install.media.stage_verified_install_set"
                ) as stage,
    ...
    def test_universal_staging_replaces_completed_older_passive_install_set(self) -> None:
        with tempfile.TemporaryDirectory() as directory_name:
            root = Path(directory_name)
            authorization = SimpleNamespace(
                raw_manifest=b"authorization",
            ...
            def replace(**values: object) -> dict[str, str]:
                self.assertEqual(values["old_bootstrap_bytes"], b"old-bootstrap")
                self.assertEqual(values["old_stage2_bytes"], b"old-stage2")
                passive.write_bytes(validated.bootstrap)
                stage2_path.write_bytes(validated.stage2.raw)
                return {"STAGE1.PKG": "replaced"}

            def activate(**_values: object) -> dict[str, str]:
                self.assertEqual(passive.read_bytes(), validated.bootstrap)
                self.assertEqual(stage2_path.read_bytes(), validated.stage2.raw)
                return {"bootstrap": "reactivated"}
    ...
    def test_universal_staging_rejects_incomplete_passive_install_set(self) -> None:
        with tempfile.TemporaryDirectory() as directory_name:
            root = Path(directory_name)
            (root / "STAGE1.PKG").write_bytes(b"bootstrap")
            preflight = SimpleNamespace(
                physical_device="/dev/test-media",
                mount_root=root.resolve(),
            )
            with (
                mock.patch("installer.universal_install.media.validate_sd_root"),
                self.assertRaisesRegex(ValueError, "incomplete or ambiguous"),
            ):
                stage_camera_bound_universal_install(
                    SimpleNamespace(),
                    root=root,
