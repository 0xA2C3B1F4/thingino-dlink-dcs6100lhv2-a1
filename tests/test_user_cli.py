...
class UserCliTests(unittest.TestCase):
    ...
    def test_universal_recovery_evacuation_preserves_pair_before_restaging(self) -> None:
        with tempfile.TemporaryDirectory() as directory_name:
            root = Path(directory_name)
            card = root / "card"
            work = root / "work"
            output = root / "private-evacuation"
            card.mkdir()
            arguments = SimpleNamespace(
                whole_device="/dev/test-card",
                mount_root=card,
    ...
    def test_universal_recovery_evacuation_parser_requires_private_output(self) -> None:
        parsed = user_cli.build_parser().parse_args(
            [
                "universal",
                "evacuate-recovery",
    ...
    def test_universal_verify_uses_uartless_session_pinned_health(self) -> None:
        parsed = user_cli.build_parser().parse_args(
            [
                "universal",
                "verify",
        ...
        self.assertIs(parsed.handler, user_cli._universal_verify)
        health = {
            "health_gate": "passed",
