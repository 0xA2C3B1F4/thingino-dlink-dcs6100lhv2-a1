...
class FinalBundleTests(unittest.TestCase):
    def test_final_kernel_bounds_the_one_megabyte_fragment_cache(self) -> None:
        for system_size in (4096, SYSTEM_FLASH_SPAN):
            with self.subTest(system_size=system_size):
                config = render_final_kernel_fragment(system_size).decode("ascii")
                lines = config.splitlines()
                self.assertIn("CONFIG_SQUASHFS_EMBEDDED=y", lines)
                self.assertEqual(
                    lines.count("CONFIG_SQUASHFS_FRAGMENT_CACHE_SIZE=1"),
                    1,
                )
