...
class Stage2Tests(unittest.TestCase):
    ...
    def _retired_v2_payload(
        ...
        kernel = test_uimage(command_line.encode("ascii"))
        fmt = ">16sII32s8sQQQ32sQQ32sIIIII32s"
        struct_size = struct.calcsize(fmt)
        system_offset = HEADER_SIZE + len(kernel)
        header = struct.pack(
            fmt,
    ...
    def test_ipv6_disabled_predecessors_are_migration_only(self) -> None:
        for old_memory in (False, True):
            with self.subTest(memory_42_22=old_memory):
                raw = self._retired_v2_payload(
                    memory_42_22=old_memory, ipv6_disabled=True
                )
                self.assertEqual(
                    validate_retired_stage2_v2_ipv6_disabled(raw).raw, raw
                )
                with self.assertRaisesRegex(ValueError, "exact final command line"):
                    validate_stage2(raw)
                damaged = bytearray(raw)
                damaged[-1] ^= 1
                with self.assertRaisesRegex(ValueError, "system digest mismatch"):
                    validate_retired_stage2_v2_ipv6_disabled(bytes(damaged))
        with self.assertRaisesRegex(ValueError, "exact final command line"):
            validate_retired_stage2_v2_ipv6_disabled(self._payload())
