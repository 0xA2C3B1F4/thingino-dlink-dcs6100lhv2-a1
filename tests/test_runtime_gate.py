...
class RuntimeGateTests(unittest.TestCase):
    def test_report_is_bound_to_bundle_and_all_required_live_tests(self) -> None:
        with tempfile.TemporaryDirectory() as directory_name:
            ...
            document = report(bundle.bundle_sha256)
            document["tests"]["native_media"] = {  # type: ignore[index]
                "stock_iq_copied": True,
                "stock_modules_copied": True,
                "thingino_iq": False,
                "thingino_modules": False,
            }
            path.write_text(json.dumps(document), encoding="utf-8")
            with self.assertRaisesRegex(runtime_gate.RuntimeGateError, "kernel-matched"):
                runtime_gate.validate_runtime_report(path, bundle)
