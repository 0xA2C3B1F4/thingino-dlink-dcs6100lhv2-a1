...
class RaptorRwdPersistentTests(unittest.TestCase):
    ...
    def test_prudynt_ring_retains_failed_start_guard(self) -> None:
        source = b"before\n\t\t" + persistent.PRUDYNT_GUARDED_START + b"after\n"
        patched = persistent.patch_prudynt_init(source)
        self.assertEqual(
            patched,
            b"before\n\t\t" + persistent.PRUDYNT_GUARDED_RING_START + b"after\n",
        )
        self.assertEqual(persistent.PRUDYNT_GUARDED_RING_START, build.PRUDYNT_GUARDED_RING_START)
        with self.assertRaises(persistent.PersistentCandidateError):
            persistent.patch_prudynt_init(patched)
        with self.assertRaises(persistent.PersistentCandidateError):
            persistent.patch_prudynt_init(source + persistent.PRUDYNT_START)
    ...
    def test_route_library_pruning_requires_no_runtime_reference(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            root = Path(name)
            library = root / "usr/lib/libnl-route-3.so.200.26.0"
            library.parent.mkdir(parents=True)
            library.write_bytes(b"fixture-route-library")
            aliases = [library.parent / filename for filename in (
                "libnl-route-3.so", "libnl-route-3.so.200",
            )]
            for alias in aliases:
                alias.symlink_to(library.name)
            consumer = root / "consumer"
            consumer.write_bytes(b"\x00libnl-route-3.so.200\x00")
            with self.assertRaisesRegex(persistent.PersistentCandidateError, "reference"):
                persistent._remove_unused_route_library(root)
            self.assertTrue(library.exists())
            self.assertTrue(all(alias.is_symlink() for alias in aliases))
            consumer.write_bytes(b"\x00libnl-3.so.200\x00")
            persistent._remove_unused_route_library(root)
            self.assertFalse(library.exists())
            self.assertTrue(all(not alias.is_symlink() for alias in aliases))
            self.assertEqual(consumer.read_bytes(), b"\x00libnl-3.so.200\x00")
    ...
    def test_universal_provenance_and_closed_services_are_separate(self) -> None:
        with tempfile.TemporaryDirectory() as name:
            ...
            rwd.unlink()
            persistent._validate_universal_tree(root, require_rwd=False)
            with self.assertRaises(persistent.PersistentCandidateError):
                persistent._validate_universal_tree(root)
            rwd.symlink_to("missing-service")
            with self.assertRaises(persistent.PersistentCandidateError):
                persistent._validate_universal_tree(root, require_rwd=False)
            rwd.unlink()
