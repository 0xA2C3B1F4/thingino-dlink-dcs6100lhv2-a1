...
def parse_elf32_mips_object(
    raw: bytes, label: str, *, expected_type: int | None = None
) -> ElfMetadata:
    """Validate one ELF32 MIPS object and return its ABI flags."""
    ...
    return ElfMetadata(flags=flags, needed=(), soname=None)


def parse_elf32_mips(raw: bytes, label: str, *, expected_type: int | None = None) -> ElfMetadata:
    """Parse the dynamic fields needed by the fail-closed shared-library gate."""

    header = parse_elf32_mips_object(raw, label, expected_type=expected_type)
    (
        _identity,
        _elf_type,
        _machine,
...
def load_vendor_bundle(directory: Path, *, catalog_path: Path = CATALOG_PATH) -> VendorBundle:
    ...
    for expected in catalog_files:
        _exact_keys(
            expected,
            {"destination", "elf", "format", "name", "required", "rootfs", "sha256", "size", "source_path"},
            "catalog file",
        )
        name = expected["name"]
        ...
        metadata: ElfMetadata | None
        if artifact_format == "elf-shared":
        ...
        elif artifact_format == "elf-relocatable":
            metadata = parse_elf32_mips_object(raw, f"vendor file {name}", expected_type=1)
            elf = _object(expected["elf"], "catalog ELF metadata")
            _exact_keys(elf, {"flags"}, "catalog ELF metadata")
            try:
                expected_flags = int(str(elf["flags"]), 0)
            except ValueError as exc:
                raise VendorBundleError("catalog ELF flags are invalid") from exc
            if metadata.flags != expected_flags:
                raise VendorBundleError(f"vendor ELF ABI mismatch: {name}")
        elif artifact_format == "opaque":
            if expected["elf"] is not None:
                raise VendorBundleError("opaque catalog file has ELF metadata")
            metadata = None
        else:
            raise VendorBundleError("public vendor catalog has an invalid format")
        artifacts.append(
            VendorArtifact(
                name=name,
