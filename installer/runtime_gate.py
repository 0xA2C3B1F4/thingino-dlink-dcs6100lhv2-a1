...
def validate_runtime_report(path: Path, bundle: VendorBundle) -> RuntimeDecision:
    ...
    if native != {
        ...
        raise RuntimeGateError("runtime report does not use kernel-matched source media")
    ...
    if audio_required and "libaudioProcess.so" not in {artifact.name for artifact in bundle.artifacts}:
        raise RuntimeGateError("AAC used libaudioProcess.so but the required stock input is missing")
