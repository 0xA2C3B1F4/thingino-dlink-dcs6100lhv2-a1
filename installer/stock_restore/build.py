...
_RESTORER_MODULES = ("runtime.inc", "verify.inc", "flash.inc", "authorization.inc")
_EXTERNAL_INCLUDES = ("generated_contract.h", "freestanding_sha256.h")
_DIRECTIVE_MARKERS = (b"#", b"%:", b"??=")
...
def read_restorer_source(source: Path = SOURCE) -> bytes:
    """Assemble the four private fragments without resolving external includes.

    Include names, spelling, count and order are fixed. Both source validation
    and compilation consume these same bytes; modules cannot bypass the source
    safety checks by remaining hidden behind preprocessor includes.
    """

    def read_regular(path: Path) -> bytes:
        if path.is_symlink() or not path.is_file():
            raise StockRestoreBuildError(f"stock restorer source is not regular: {path.name}")
        try:
            raw = path.read_bytes()
        except OSError as exc:
            raise StockRestoreBuildError(f"stock restorer source cannot be read: {path.name}") from exc
        # These fragments use neither physical line splices nor trigraphs.
        # Reject them instead of letting preprocessing create a hidden marker
        # such as a %: token split over two physical lines.
        if b"\\\n" in raw or b"\\\r" in raw or b"??" in raw:
            raise StockRestoreBuildError("stock restorer source contains a continuation or trigraph")
        return raw

    root = read_regular(source)
    lines = root.splitlines(keepends=True)
    expected = (*_EXTERNAL_INCLUDES, *_RESTORER_MODULES)
    # This fixed source format deliberately rejects directive markers even in
    # comments/strings unless the entire physical line has the canonical form.
    # Do not interpret C whitespace, comments, digraphs, trigraphs or splices:
    # those could otherwise conceal an include from validation but not clang.
    directives = [line for line in lines if any(marker in line for marker in _DIRECTIVE_MARKERS)]
    if directives != [f'#include "{name}"\n'.encode("ascii") for name in expected]:
        raise StockRestoreBuildError("stock restorer source include names or order differ")
    modules: dict[bytes, bytes] = {}
    for name in _RESTORER_MODULES:
        raw = read_regular(source.parent / name)
        if any(
            any(marker in line for marker in _DIRECTIVE_MARKERS)
            and (not line.startswith(b"#define ") or line.count(b"#") != 1
                 or b"%:" in line or b"??=" in line)
            for line in raw.splitlines()
        ):
            raise StockRestoreBuildError("stock restorer module contains an include or unsupported directive")
        modules[f'#include "{name}"\n'.encode("ascii")] = raw
    return b"".join(modules.get(line, line) for line in lines)
...
def build_stock_restore_root(
    *,
    ...
    source_raw = read_restorer_source()
