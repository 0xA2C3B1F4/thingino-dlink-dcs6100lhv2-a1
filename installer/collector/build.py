...
def render_contract(
    *, mmc_module: bytes, capture_mode: str = "existing-recovery"
    ...
    if catalog.get("schema_version") != 2 or catalog.get("target") != _target_document():
    ...
    if not isinstance(files, list) or len(files) != 7:
        raise CollectorBuildError("collector requires the exact seven-file catalog")
    expected_names = (
        "libimp.so",
        "libalog.so",
        "libsysutils.so",
    ...
    if tuple(entry.get("required") for entry in files) != (True,) * 7:
