...
def _universal_evacuate_recovery(
    ...
    atomic_write = getattr(facade, "atomic_write")
    create_preflight = getattr(facade, "create_preflight_document")
    evacuate = getattr(facade, "evacuate_existing_stock_backups")
    json_module = getattr(facade, "json")
    load_preflight = getattr(facade, "load_media_preflight")
    ...
    work_dir.mkdir(parents=True, exist_ok=True, mode=0o700)
    work_dir.chmod(0o700)
    preflight_path = work_dir / "universal-evacuation-preflight.private.json"
    atomic_write(
        preflight_path,
        (json_module.dumps(preflight_document, indent=2, sort_keys=True) + "\n").encode(),
        mode=0o600,
    )
    preflight = load_preflight(preflight_path, expected_root=mount_root)
    device_confirmation = arguments.confirm_physical_device
    if device_confirmation is None and not arguments.json:
        device_confirmation = input(
            f"Type the exact whole device {preflight.physical_device} to confirm recovery evacuation: "
        ).strip()
    if device_confirmation is None:
        raise UserInstallerError("JSON mode requires --confirm-physical-device")
    ...
    return _document(
        "universal evacuate-recovery",
        ok=True,
        phase="completed-recovery-checkpoint-evacuated",
        next_command="thingino-dlink universal stage",
        result={
            "evacuated_files": evacuated,
            "evacuation_dir": str(arguments.output_dir),
            "nor_written_by_host": False,
            "physical_device": preflight.physical_device,
            "safe_next_action": "stage-new-camera-bound-universal-card",
            "sd_modified": True,
            "write_set": [],
        },
    )
...
def _universal_verify(
    facade: object, arguments: argparse.Namespace
) -> dict[str, object]:
    _document = getattr(facade, "_document")
    Path = getattr(facade, "Path")
    UserInstallerError = getattr(facade, "UserInstallerError")
    ensure_station_pin = getattr(facade, "ensure_uartless_station_host_pin")
    ...
    health = prove_thingino_health(session_dir=arguments.session_dir)
    return _document(
        "universal verify",
        ok=True,
