...
def ensure_uartless_provisioning_session(
    *,
    ...
    try:
        ...
        host_public_key = _generate_dropbear_host_key(
            destination=host / "dropbear_ed25519_host_key",
            dropbearkey=dropbearkey,
        )
        _write(
            host / "station_known_hosts",
            station_mdns_name.encode("ascii") + b" " + host_public_key,
        )
        manifest = {
            "ap_address": None,
...
def _generate_dropbear_host_key(*, destination: Path, dropbearkey: Path) -> bytes:
    _run(
        [str(dropbearkey), "-t", "ed25519", "-f", str(destination)],
        "Dropbear host-key generation",
    )
    public_key = _public_key_from_output(
        _run(
...
def _ensure_uartless_dropbear_host_key(
    *, host_dir: Path, dropbearkey: Path, station_mdns_name: str
) -> None:
    ...
    if destination.exists() or destination.is_symlink():
        ...
        try:
            ...
            public_key = _generate_dropbear_host_key(
            ...
            shutil.rmtree(temporary)
    expected = station_mdns_name.encode("ascii") + b" " + public_key
    station_pin = host_dir / "station_known_hosts"
    if station_pin.exists() or station_pin.is_symlink():
        if _read_private(station_pin, "station known-host binding", 4096) != expected:
            raise RecoveryApSessionError("station known-host binding differs")
    else:
        _write(station_pin, expected)


def ensure_uartless_station_host_pin(
    *, session_dir: Path, dropbearkey: Path
) -> bool:
    """Create or validate the final-station pin derived from the provisioned key."""

    from .host import load_host_session

    if dropbearkey.is_symlink() or not dropbearkey.is_file() or not os.access(
        dropbearkey, os.X_OK
    ):
        raise RecoveryApSessionError("dropbearkey is not an executable regular file")
    session = load_host_session(session_dir)
    if session.session_kind != "uartless-functional-provisioning":
        raise RecoveryApSessionError("session is not UARTless functional provisioning")
    station_pin = session_dir / "host/station_known_hosts"
    created = not station_pin.exists()
    _ensure_uartless_dropbear_host_key(
        host_dir=session_dir / "host",
        dropbearkey=dropbearkey,
        station_mdns_name=session.station_mdns_name,
    )
    return created
