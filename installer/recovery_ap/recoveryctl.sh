...
vendor-export)
	...
	check_vendor() {
		...
		expected_size=$2
		expected_sha256=$3
		source=/run/stock-mtd3/lib/$relative
		[ -f "$source" ] && [ ! -L "$source" ] || exit 2
		[ "$(wc -c <"$source")" -eq "$expected_size" ] || exit 2
		actual_sha256=$(sha256sum "$source")
		actual_sha256=${actual_sha256%% *}
		[ "$actual_sha256" = "$expected_sha256" ] || exit 2
		cp "$source" "/run/vendor-export/$relative"
	}
	check_vendor libimp.so 1146852 14b18d23964f18b63cef3a32ca7a6dc7ae8ee6ebb001c646ffa0ace72c2273fe
	check_vendor libalog.so 36044 40fd7eb9237772f705a92e9792325f07f0fe022479923f8dc67653cc11450ea1
	check_vendor libsysutils.so 30020 befca6166d2e25b749cc9fff4798332f42a2d997b00d4aee713358d803353b79
	if [ -e /run/stock-mtd3/lib/libaudioProcess.so ]; then
		check_vendor libaudioProcess.so 697757 0f03bee6156b3a570c4af8cc199a53b1302fb4450570a53222107ff498a3ae72
	fi
