...
def prove_thingino_health(
    *, session_dir: Path, expected_mtd3_sha256: str | None = None
    ...
    if expected_mtd3_sha256 is None:
        mdns_name, host = resolve_recovery_ap_station(
            session_dir,
            allow_uartless_station=uartless_station,
        )
        hosts = (host,)
    else:
        mdns_name, hosts = resolve_recovery_ap_station_candidates(
            session_dir,
            allow_uartless_station=uartless_station,
        )
    payload = secrets.token_bytes(4096)
    # A fresh SD install writes flash and reboots before management can start.
    # Allow that complete sequence, while retaining bounded connection attempts.
