...
result_dir=/result
empty_vendor_site=$workspace/stock-restore-empty-vendor-site
profile=dlink_dcs6100lhv2_a1_t31n_os02g10_rtl8188fu
source_epoch=1786006608
restore_fragment="\$(BR2_EXTERNAL_THINGINO_PATH)/configs/cameras-exp/$profile/stock-restore-kernel.fragment"

test -f "$source_input/dcs6100-source-preparation.json"
test -f "$source_input/configs/cameras-exp/$profile/stock-restore-kernel.fragment"
...
if ! test -d "$source_dir/.git"; then
	test -z "$(find "$workspace" -mindepth 1 -maxdepth 1 ! -name lost+found -print -quit)"
...
if test "${initial_config:-0}" = 1; then
	...
	sed -i "s|^BR2_LINUX_KERNEL_CONFIG_FRAGMENT_FILES=.*$|BR2_LINUX_KERNEL_CONFIG_FRAGMENT_FILES=\"$restore_fragment\"|" "$output_dir/.config"
...
grep -Fx "BR2_LINUX_KERNEL_CONFIG_FRAGMENT_FILES=\"$restore_fragment\"" "$output_dir/.config" >/dev/null
...
install -m 0400 "$output_dir/images/uImage" "$result_dir/stock-restore-kernel.uimage"
install -m 0400 "$linux_config" "$result_dir/stock-restore-linux.config"
