...
base_fragment_rel="\$(BR2_EXTERNAL_THINGINO_PATH)/configs/cameras-exp/$profile/kernel.fragment"
...
test -f "$fragment_dir/kernel.fragment"
...
build_kernel() {
	...
	sed -i \
		"s|^BR2_LINUX_KERNEL_CONFIG_FRAGMENT_FILES=.*$|BR2_LINUX_KERNEL_CONFIG_FRAGMENT_FILES=\"$fragment_list\"|" \
		"$output_dir/.config"
	grep -Fx "BR2_LINUX_KERNEL_CONFIG_FRAGMENT_FILES=\"$fragment_list\"" \
	...
	for required_fragment in "$fragment_dir/kernel.fragment" "$fragment_file"; do
		while IFS= read -r required; do
			case "$required" in
			CONFIG_*=*|'# CONFIG_'*' is not set')
				grep -Fx "$required" "$linux_config" >/dev/null
				;;
			esac
