...
build_script="$script_root/scripts/container_build_stock_restore_kernel.sh"
[ -f "$build_script" ] && [ ! -L "$build_script" ] || {
  echo "repository stock-restore kernel build script is missing" >&2
  exit 1
}
...
docker run --rm --network none --platform linux/arm64 \
  ...
  'truncate -s 20G "/host/$1" && mkfs.ext4 -q -F -L dcs6100-restore "/host/$1"' \
