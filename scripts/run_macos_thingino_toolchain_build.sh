...
usage() {
  echo "usage: $0 --task-scratch-root DIR --builder-lock DIR --builder-image IMAGE --container-name NAME --source-checkout DIR --download-cache FILE --workspace-image FILE --result DIR" >&2
  exit 2
}
...
for value in "$task_scratch_root" "$builder_lock" "$builder_image" \
  "$container_name" "$source_checkout" "$download_cache" \
...
[ -f "$download_cache" ] && [ ! -L "$download_cache" ] || {
  echo "toolchain download cache is missing or invalid" >&2
  exit 1
}
...
build_script="$script_root/scripts/container_build_thingino_toolchain.sh"
[ -f "$build_script" ] && [ ! -L "$build_script" ] || {
  echo "repository toolchain build script is missing" >&2
  exit 1
}
...
docker run --rm --network none --platform linux/arm64 \
  ...
  'truncate -s 40G "/host/$1" && mkfs.ext4 -q -F -L dcs6100-tc-build "/host/$1"' \
  toolchain-build-workspace "$workspace_name"

docker run --rm --name "$container_name" --network none --privileged \
  --platform linux/arm64 --entrypoint /bin/sh \
  -v "$build_script:/build.sh:ro" \
  -v "$source_checkout:/input/source:ro" \
  -v "$download_cache:/input/toolchain-download-cache.tar:ro" \
